...
class WgpuPbrPipelineCache {
  ...
  public static function compileWgpuPbrPipeline(state:WgpuRenderState, key:WgpuPbrDefineKey, format:flighthq._internal.dom.GPUTextureFormat, blended:Bool = false, skinned:Bool = false, ?colorAdjustmentFeature:Null<WgpuColorAdjustmentMaterialFeature>):WgpuPbrPipeline {
    ...
    module = flighthq._internal.backend.WebGpuDeviceBackend.call(device, 'createShaderModule', cast ([{ code: (cast getWgpuPbrModuleSourceForKey(({ final __callArgument0:Dynamic = key; __callArgument0; }), (cast skinned : Bool), (cast (cast getWgpuSkinningAdapter(({ final __callArgument1:Dynamic = state; __callArgument1; })) : Null<WgpuSkinningAdapter>) : Dynamic), ({ final __callArgument3:Dynamic = colorAdjustmentFeature; __callArgument3; })) : String) }] : Array<Dynamic>));
    ...
    return cast (cast createWgpuMeshPipeline(({ final __callArgument8:Dynamic = state; __callArgument8; }), ({ final __callArgument11:Dynamic = { blended: blended, doubleSided: _Runtime.field(key, 'doubleSided'), format: format, materialBindGroupLayout: materialBindGroupLayout, module: module, pbrSampleBindGroupLayout: (cast ensureWgpuPbrSampleLayout(({ final __callArgument9:Dynamic = state; __callArgument9; })) : flighthq._internal.dom.GPUBindGroupLayout), skinned: skinned }; __callArgument11; })) : WgpuMeshPipeline);
    return cast null;
  }
