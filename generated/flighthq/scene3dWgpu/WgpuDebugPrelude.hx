...
class WgpuDebugPrelude {
  ...
  public static function compileWgpuDebugPipeline(state:WgpuRenderState, key:WgpuDebugDefineKey, format:Dynamic, blended:Dynamic = false):WgpuDebugPipeline {
    ...
    return cast _Runtime.callValue(createWgpuMeshPipeline, cast ([state, { blended: blended, doubleSided: false, format: format, materialBindGroupLayout: materialBindGroupLayout, module: module }] : Array<Dynamic>));
    return cast null;
  }
  ...
  public static function ensureWgpuDebugPipeline(state:WgpuRenderState, key:WgpuDebugDefineKey, format:Dynamic):WgpuDebugPipeline {
    return cast _Runtime.callValue(ensureWgpuScene3DPipeline, cast ([state, 'debug:' + Std.string(format) + '|' + Std.string(_Runtime.callValue(buildWgpuDebugDefineKey, cast ([key] : Array<Dynamic>))) + '', function(blended:Dynamic) return _Runtime.callValue(compileWgpuDebugPipeline, cast ([state, key, format, blended] : Array<Dynamic>))] : Array<Dynamic>));
    return cast null;
  }
