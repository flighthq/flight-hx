...
class WgpuBevelEffect {
  ...
  public static function applyBevelEffectToWgpu(state:WgpuRenderState, source:WgpuRenderTarget, dest:WgpuRenderTarget, pool:WgpuRenderTargetPool, effect:BevelEffect):Void {
    ...
    _Runtime.callValue(WgpuBevelEffect.applyWgpuBevelCompositePass__wgpuBevelEffect, cast ([state, blurred, src, dst, { offsetX: _Runtime.divideNumbers(offsetX, _Runtime.field(source, 'width')), offsetY: _Runtime.divideNumbers(-offsetY, _Runtime.field(source, 'height')), highlightColor: highlightColor, highlightAlpha: highlightAlpha, shadowColor: shadowColor, shadowAlpha: shadowAlpha, intensity: strength, clipMode: ((cast _Runtime.strictEquals(bevelType, 'inner') : Bool) ? (cast 1.0 : Dynamic) : (cast ((cast _Runtime.strictEquals(bevelType, 'outer') : Bool) ? (cast 2.0 : Dynamic) : (cast 0.0 : Dynamic)) : Dynamic)) }] : Array<Dynamic>));
