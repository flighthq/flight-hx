...
class Shortcut {
  ...
  public static function _isMacOS__shortcut(?platform:String):Bool {
    ...
    if (_Runtime.truthy(!_Runtime.strictEquals(platform, _Runtime.UNDEFINED))) { return cast _Runtime.callProperty(_Runtime.regexp('^mac', 'i'), 'test', cast ([platform] : Array<Dynamic>)); }
  ...
  return cast { reason: 'empty', token: '' };
    ...
    for (token in _Runtime.iterable(tokens)) {
      ...
      if (_Runtime.truthy(!_Runtime.strictEquals(mod, _Runtime.UNDEFINED))) {
  ...
  return cast { reason: 'missing-key', token: '' };
    ...
    if (_Runtime.truthy(_Runtime.strictEquals(canonicalKey, _Runtime.UNDEFINED))) {
