// Generated by flight-hx. Do not edit.
package flighthq.shading;

import Math as HxMath;
import flighthq._internal._Runtime;
import flighthq.types.ModifierSlot;
import flighthq.types._internal._ModifierSlotValues.ModifierSlotValue;

class IsBuiltInModifierSlot {
  public static function isBuiltInModifierSlot(value:String):Bool {
    return cast ((cast IsBuiltInModifierSlot.BUILT_IN_SLOTS__isBuiltInModifierSlot : flighthq._internal._Set).has(value));
    return cast null;
  }

  public static final BUILT_IN_SLOTS__isBuiltInModifierSlot:Dynamic = _Runtime.construct(flighthq._internal._HostValueLut.get('Set'), [cast ([ModifierSlotValue.Diffuse, ModifierSlotValue.Effect, ModifierSlotValue.Emissive, ModifierSlotValue.Normal, ModifierSlotValue.Specular, ModifierSlotValue.Vertex] : Array<Dynamic>)]);
}
