...
class GlFxaaEffect {
  ...
  public static function applyFxaaEffectToGl(state:GlRenderState, source:GlRenderTarget, dest:GlRenderTarget, effect:FxaaEffect):Void {
    ...
    program = (cast getGlEffectProgram((cast state : GlRenderState), (cast 'antialiasing.fxaa' : String), (cast GlFxaaEffect.FXAA_FRAGMENT_SRC__glFxaaEffect : String)) : GlFullscreenProgram);
  ...
  public static final defaultGlFxaaEffectRunner:GlRenderEffectRunner = function(ctx:GlRenderEffectContext, effect:RenderEffect):Void {
    applyFxaaEffectToGl((cast _Runtime.field(ctx, 'state') : GlRenderState), (cast _Runtime.field(ctx, 'source') : GlRenderTarget), (cast _Runtime.field(ctx, 'dest') : GlRenderTarget), (cast (cast effect : FxaaEffect) : FxaaEffect));
  };

  public static function registerGlFxaaEffect(state:GlRenderState):Void {
    registerGlRenderEffect((cast state : GlRenderState), (cast 'FxaaEffect' : String), (cast defaultGlFxaaEffectRunner : GlRenderEffectRunner));
  }
