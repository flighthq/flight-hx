...
class GlBevelEffect {
  ...
  public static function applyBevelEffectToGl(state:GlRenderState, source:GlRenderTarget, dest:GlRenderTarget, pool:GlRenderTargetPool, effect:BevelEffect):Void {
    ...
    GlBevelEffect.applyGlBevelCompositePass__glBevelEffect(({ final __callArgument22:Dynamic = state; __callArgument22; }), ({ final __callArgument23:Dynamic = blurred; __callArgument23; }), ({ final __callArgument24:Dynamic = src; __callArgument24; }), ({ final __callArgument25:Dynamic = dst; __callArgument25; }), ({ final __callArgument26:Dynamic = { offsetX: (offsetX / (cast src : GlRenderTarget).width), offsetY: (-offsetY / (cast src : GlRenderTarget).height), highlightColor: highlightColor, highlightAlpha: highlightAlpha, shadowColor: shadowColor, shadowAlpha: shadowAlpha, intensity: strength, clipMode: ((cast _Runtime.strictEquals(bevelType, 'inner') : Bool) ? (cast 1.0 : Dynamic) : (cast ((cast _Runtime.strictEquals(bevelType, 'outer') : Bool) ? (cast 2.0 : Dynamic) : (cast 0.0 : Dynamic)) : Dynamic)) }; __callArgument26; }));
    if ((cast _Runtime.strictEquals(sourceMode, 'knockout') : Bool)) { applyGlEffectErasePass(({ final __callArgument27:Dynamic = state; __callArgument27; }), ({ final __callArgument28:Dynamic = src; __callArgument28; }), ({ final __callArgument29:Dynamic = dst; __callArgument29; })); }
    releaseGlRenderTarget(({ final __callArgument30:Dynamic = pool; __callArgument30; }), ({ final __callArgument31:Dynamic = s0; __callArgument31; }));
    releaseGlRenderTarget(({ final __callArgument32:Dynamic = pool; __callArgument32; }), ({ final __callArgument33:Dynamic = s1; __callArgument33; }));
    releaseGlRenderTarget(({ final __callArgument34:Dynamic = pool; __callArgument34; }), ({ final __callArgument35:Dynamic = s2; __callArgument35; }));
  }
  ...
  public static function applyGlBevelCompositePass__glBevelEffect(state:GlRenderState, field:GlRenderTarget, source:GlRenderTarget, dest:GlRenderTarget, params:BevelCompositeParams__glBevelEffect):Void {
    ...
    loc = (cast GlBevelEffect.getGlBevelCompositeShader__glBevelEffect(({ final __callArgument38:Dynamic = state; __callArgument38; })) : BevelCompositeLocations__glBevelEffect);
    drawGlFullscreenPass(({ final __callArgument39:Dynamic = state; __callArgument39; }), ({ final __callArgument40:Dynamic = loc; __callArgument40; }), ({ final __callArgument41:Dynamic = cast ([(cast field : GlRenderTarget).texture, (cast source : GlRenderTarget).texture] : Array<Dynamic>); __callArgument41; }), ({ final __callArgument42:Dynamic = dest; __callArgument42; }), ({ final __callArgument43:Dynamic = function(__unused2:flighthq._internal.dom.WebGL2RenderingContext, __unused3:GlFullscreenProgram):Void { _Runtime.callValue(function(gl:flighthq._internal.dom.WebGL2RenderingContext, __unused1:GlFullscreenProgram):Void {
    ...
    }, cast ([__unused2] : Array<Dynamic>)); }; __callArgument43; }));
  }
  ...
  public static function getGlBevelCompositeShader__glBevelEffect(state:GlRenderState):BevelCompositeLocations__glBevelEffect {
    ...
    if ((cast _Runtime.strictEquals(loc, _Runtime.field(_Runtime, 'UNDEFINED')) : Bool)) {
      ...
      var base:GlFullscreenProgram = (cast compileGlFullscreenProgram(({ final __callArgument44:Dynamic = gl; __callArgument44; }), (cast GlBevelEffect.BEVEL_COMPOSITE_FRAGMENT_SRC__glBevelEffect : String)) : GlFullscreenProgram);
