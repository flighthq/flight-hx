...
class AsepriteSerialize {
  public static function dataToMeta__asepriteSerialize(data:SpritesheetData, existing:Dynamic):AsepriteMeta {
    ...
    tags = _Runtime.callProperty(_Runtime.field(data, 'animations'), 'map', cast ([function(anim:Dynamic, i:Dynamic) {
      ...
      return cast _Runtime.mergeObjects([{ direction: _Runtime.field(anim, 'direction') }, { from: ((cast ((cast firstIdx : Float) >= (cast 0.0 : Float)) : Bool) ? (cast firstIdx : Dynamic) : (cast 0.0 : Dynamic)) }, { name: _Runtime.field(anim, 'name') }, { to: ((cast ((cast lastIdx : Float) >= (cast 0.0 : Float)) : Bool) ? (cast lastIdx : Dynamic) : (cast 0.0 : Dynamic)) }, ((cast !_Runtime.strictEquals(_Runtime.optionalField(_Runtime.optionalIndex(_Runtime.field(existing, 'frameTags'), i), 'color'), _Runtime.field(_Runtime, 'UNDEFINED')) : Bool) ? (cast { color: _Runtime.field(_Runtime.getIndex(_Runtime.field(existing, 'frameTags'), i), 'color') } : Dynamic) : (cast {  } : Dynamic))]);
    }] : Array<Dynamic>));
  ...
  public static function resolveFrameDuration__asepriteSerialize(data:SpritesheetData, frameName:String):Float {
    for (anim in _Runtime.iterable(_Runtime.field(data, 'animations'))) {
      ...
      if ((cast !_Runtime.strictEquals(_Runtime.field(anim, 'frameDurations'), null) : Bool)) { return cast _Runtime.coalesce(_Runtime.getIndex(_Runtime.field(anim, 'frameDurations'), idx), function():Dynamic return cast _Runtime.field(anim, 'frameDuration')); }
