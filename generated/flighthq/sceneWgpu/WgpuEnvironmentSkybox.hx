...
class WgpuEnvironmentSkybox {
  ...
  public static function ensureWgpuSkyboxPipeline__wgpuEnvironmentSkybox(state:WgpuRenderState, format:Dynamic):WgpuSkybox__wgpuEnvironmentSkybox {
    ...
    uniformBindGroupLayout = _Runtime.callProperty(device, 'createBindGroupLayout', cast ([{ entries: cast ([{ binding: 0.0, visibility: (Std.int(_Runtime.field(_Runtime.globalValue('GPUShaderStage'), 'VERTEX')) | Std.int(_Runtime.field(_Runtime.globalValue('GPUShaderStage'), 'FRAGMENT'))), buffer: { type: 'uniform' } }] : Array<Dynamic>) }] : Array<Dynamic>));
    cubeBindGroupLayout = _Runtime.callProperty(device, 'createBindGroupLayout', cast ([{ entries: cast ([{ binding: 0.0, visibility: _Runtime.field(_Runtime.globalValue('GPUShaderStage'), 'FRAGMENT'), texture: { sampleType: 'float', viewDimension: 'cube' } }, { binding: 1.0, visibility: _Runtime.field(_Runtime.globalValue('GPUShaderStage'), 'FRAGMENT'), sampler: { type: 'filtering' } }] : Array<Dynamic>) }] : Array<Dynamic>));
    ...
    uniformBuffer = _Runtime.callProperty(device, 'createBuffer', cast ([{ size: WgpuEnvironmentSkybox.SKYBOX_UNIFORM_BYTES__wgpuEnvironmentSkybox, usage: (Std.int(_Runtime.field(_Runtime.globalValue('GPUBufferUsage'), 'UNIFORM')) | Std.int(_Runtime.field(_Runtime.globalValue('GPUBufferUsage'), 'COPY_DST'))) }] : Array<Dynamic>));
