      ...
      while ((cast ((cast row : Float) < (cast rows : Float)) : Bool)) {
        {
          ...
          while ((cast ((cast column : Float) < (cast columns : Float)) : Bool)) {
            _Runtime.callProperty(atlas.regions, 'push', cast ([(cast createTextureAtlasRegion(({ final __callArgument0:Dynamic = { height: frameHeight, id: id, name: '' + Std.string(namePrefix) + '' + Std.string(id) + '', width: frameWidth, x: (marginX + (column * (frameWidth + spacingX))), y: (marginY + (row * (frameHeight + spacingY))) }; __callArgument0; })) : TextureAtlasRegion)] : Array<Dynamic>));
            id++;
            column++;
          }
        }
