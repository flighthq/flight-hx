...
class ExplainWgpuTextureResolution {
  public static function explainWgpuTextureResolution(state:WgpuRenderState, texture:TextureLike):TextureResolutionExplanation {
    ...
    return cast { kind: kind, status: ((cast _Runtime.strictEquals(({ final __structural6 = ((cast (cast (cast (cast (cast getWgpuRenderStateRuntime(({ final __callArgument4:Dynamic = state; __callArgument4; })) : WgpuRenderStateRuntime) : { var registries:WgpuRenderRegistries; }).registries : { var textureResolvers:KeyedTable<WgpuTextureResolver>; }).textureResolvers : KeyedTable<WgpuTextureResolver>).entries : flighthq._internal._Map<String, RegistryTableEntry<WgpuTextureResolver>>).get(kind)); __structural6 == null ? _Runtime.UNDEFINED : (cast __structural6 : { var state:String; }).state; }), (cast RegistryEntryStateValue : { var Bound:String; var Tombstoned:String; }).Bound) : Bool) ? (cast 'registered' : Dynamic) : (cast 'missing-resolver' : Dynamic)) };
    return cast null;
  }
}
