...
class CleanPath {
  ...
  public static function collapseClosedSeam__cleanPath(kept:Array<Float>, toleranceSq:Float):Void {
    ...
    while ((cast ((cast changed : Bool) && (cast ((cast (_Runtime.toInt32(_Runtime.field(kept, 'length')) >> 1) : Float) > (cast 3.0 : Float)) : Bool)) : Bool)) {
      ...
      if ((cast (cast CleanPath.isMiddleRemovable__cleanPath(({ final __callArgument5:Dynamic = kept; __callArgument5; }), (cast (last - 2.0) : Float), (cast (last - 1.0) : Float), (cast 0.0 : Float), (cast toleranceSq : Float)) : Bool) : Bool)) {
        _Runtime.setLength(kept, (kept.length - 2.0));
        (changed = cast (true : Dynamic));
        continue;
      }
      if ((cast (cast CleanPath.isMiddleRemovable__cleanPath(({ final __callArgument6:Dynamic = kept; __callArgument6; }), (cast (last - 1.0) : Float), (cast 0.0 : Float), (cast 1.0 : Float), (cast toleranceSq : Float)) : Bool) : Bool)) {
