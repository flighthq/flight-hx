...
class CleanPath {
    ...
    for (contour in _Runtime.iterable(contours)) {
      ...
      var kept:Array<Float> = (cast cast ([] : Array<Dynamic>));
      {
        ...
        while ((cast ((cast i : Float) < (cast count : Float)) : Bool)) {
          CleanPath.pushCleanVertex__cleanPath(({ final __callArgument4:Dynamic = kept; __callArgument4; }), (cast flighthq._internal._StaticIndex.readFloatArrayTyped((cast contour : Array<Float>), (cast (i * 2.0) : Float)) : Float), (cast flighthq._internal._StaticIndex.readFloatArrayTyped((cast contour : Array<Float>), (cast ((i * 2.0) + 1.0) : Float)) : Float), (cast toleranceSq : Float));
          i++;
        }
      }
      if ((cast closed : Bool)) { CleanPath.collapseClosedSeam__cleanPath(({ final __callArgument6:Dynamic = kept; __callArgument6; }), (cast toleranceSq : Float)); }
  ...
  public static function collapseClosedSeam__cleanPath(kept:Array<Float>, toleranceSq:Float):Void {
    ...
    while ((cast ((cast changed : Bool) && (cast ((cast (_Runtime.toInt32(_Runtime.field(kept, 'length')) >> 1) : Float) > (cast 3.0 : Float)) : Bool)) : Bool)) {
      ...
      if ((cast (cast CleanPath.isMiddleRemovable__cleanPath(({ final __callArgument8:Dynamic = kept; __callArgument8; }), (cast (last - 2.0) : Float), (cast (last - 1.0) : Float), (cast 0.0 : Float), (cast toleranceSq : Float)) : Bool) : Bool)) {
        _Runtime.setLength(kept, (kept.length - 2.0));
        (changed = cast (true : Dynamic));
        continue;
      }
      if ((cast (cast CleanPath.isMiddleRemovable__cleanPath(({ final __callArgument10:Dynamic = kept; __callArgument10; }), (cast (last - 1.0) : Float), (cast 0.0 : Float), (cast 1.0 : Float), (cast toleranceSq : Float)) : Bool) : Bool)) {
