...
class ExplainDomTextureResolution {
  public static function explainDomTextureResolution(state:DomRenderState, texture:Texture):TextureResolutionExplanation {
    ...
    return cast { kind: kind, status: ((cast _Runtime.strictEquals(({ final __structural3 = ((cast (cast (cast (cast (cast getDomRenderStateRuntime(({ final __callArgument2:Dynamic = state; __callArgument2; })) : DomRenderStateRuntime) : { var registries:DomRenderRegistries; }).registries : { var textureResolvers:KeyedTable<DomTextureResolver>; }).textureResolvers : KeyedTable<DomTextureResolver>).entries : flighthq._internal._Map<String, RegistryTableEntry<DomTextureResolver>>).get(kind)); __structural3 == null ? _Runtime.UNDEFINED : (cast __structural3 : { var state:String; }).state; }), (cast RegistryEntryStateValue : { var Bound:String; var Tombstoned:String; }).Bound) : Bool) ? (cast 'registered' : Dynamic) : (cast 'missing-resolver' : Dynamic)) };
    return cast null;
  }
}
