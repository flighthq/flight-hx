...
class DomShape {
  ...
  public static function drawDomShape(state:DomRenderState, renderProxy:RenderProxy2D):Void {
    ...
    if ((cast !_Runtime.strictEquals(_Runtime.field(state, 'domCssFilterResolver'), null) : Bool)) {
      (#if js ((cast (#if js (cast _Runtime.field(data, 'canvas') : flighthq._internal.dom.HTMLCanvasElement).style #else _Runtime.field(_Runtime.field(data, 'canvas'), 'style') #end) : flighthq._internal.dom.CSSStyleDeclaration).filter = _Runtime.coalesce(_Runtime.callProperty(state, 'domCssFilterResolver', cast ([renderProxy] : Array<Dynamic>)), function():Dynamic return cast '')) #else _Runtime.setField((#if js (cast _Runtime.field(data, 'canvas') : flighthq._internal.dom.HTMLCanvasElement).style #else _Runtime.field(_Runtime.field(data, 'canvas'), 'style') #end), 'filter', _Runtime.coalesce(_Runtime.callProperty(state, 'domCssFilterResolver', cast ([renderProxy] : Array<Dynamic>)), function():Dynamic return cast '')) #end);
    }
