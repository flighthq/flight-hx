// Generated by flight-hx. Do not edit.
package flighthq.bitmap;

import Math as HxMath;
import flighthq._internal._Runtime;
import flighthq.types.Bitmap;

@:noCompletion
class BitmapCoverage {
  public static function getBitmapCoverage(source:Bitmap, backgroundColor:Float, channelTolerance:Float = 0.0):Float {
    var br:Float = cast _Runtime.UNDEFINED;
    var bg:Float = cast _Runtime.UNDEFINED;
    var bb:Float = cast _Runtime.UNDEFINED;
    var ba:Float = cast _Runtime.UNDEFINED;
    var data:flighthq._internal._UInt8ClampedArray = cast _Runtime.UNDEFINED;
    var totalPixels:Float = cast _Runtime.UNDEFINED;
    var covered:Float = cast _Runtime.UNDEFINED;
    br = (_Runtime.toInt32(_Runtime.unsignedShiftRight(_Runtime.toInt32(backgroundColor), 24)) & 255);
    bg = (_Runtime.toInt32((_Runtime.toInt32(backgroundColor) >> 16)) & 255);
    bb = (_Runtime.toInt32((_Runtime.toInt32(backgroundColor) >> 8)) & 255);
    ba = (_Runtime.toInt32(backgroundColor) & 255);
    data = source.data;
    totalPixels = (source.width * source.height);
    if ((cast _Runtime.strictEquals(totalPixels, 0.0) : Bool)) { return cast 0.0; }
    covered = 0.0;
    {
      var i:Float = 0.0;
      while ((cast ((cast i : Float) < (cast _Runtime.field(data, 'length') : Float)) : Bool)) {
        if ((cast ((cast ((cast ((cast ((cast HxMath.abs((flighthq._internal._StaticIndex.readUint8ClampedArrayTyped((cast data : flighthq._internal._UInt8ClampedArray), (cast i : Float)) - br)) : Float) > (cast channelTolerance : Float)) : Bool) || (cast ((cast HxMath.abs((flighthq._internal._StaticIndex.readUint8ClampedArrayTyped((cast data : flighthq._internal._UInt8ClampedArray), (cast (i + 1.0) : Float)) - bg)) : Float) > (cast channelTolerance : Float)) : Bool)) : Bool) || (cast ((cast HxMath.abs((flighthq._internal._StaticIndex.readUint8ClampedArrayTyped((cast data : flighthq._internal._UInt8ClampedArray), (cast (i + 2.0) : Float)) - bb)) : Float) > (cast channelTolerance : Float)) : Bool)) : Bool) || (cast ((cast HxMath.abs((flighthq._internal._StaticIndex.readUint8ClampedArrayTyped((cast data : flighthq._internal._UInt8ClampedArray), (cast (i + 3.0) : Float)) - ba)) : Float) > (cast channelTolerance : Float)) : Bool)) : Bool)) {
          covered++;
        }
        (i = cast ((i + 4.0) : Dynamic));
      }
    }
    return cast (covered / totalPixels);
    return cast null;
  }
}
