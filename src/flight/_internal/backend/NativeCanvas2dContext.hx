...
class NativeCanvas2dContext {
  ...
  public function roundRect(x:Float, y:Float, width:Float, height:Float, radius:Dynamic):Void {
    final left = HxMath.min(x, x + width);
    final right = HxMath.max(x, x + width);
    final top = HxMath.min(y, y + height);
    final bottom = HxMath.max(y, y + height);
    final r = HxMath.min(HxMath.abs((radius : Float)), HxMath.min((right - left) / 2, (bottom - top) / 2));
    final ctx = context();
    if (r <= 0) {
    ...
    ctx.arc(right - r, top + r, r, -HxMath.PI / 2, 0);
    ctx.lineTo(right, bottom - r);
    ctx.arc(right - r, bottom - r, r, 0, HxMath.PI / 2);
    ctx.lineTo(left + r, bottom);
    ctx.arc(left + r, bottom - r, r, HxMath.PI / 2, HxMath.PI);
    ctx.lineTo(left, top + r);
    ctx.arc(left + r, top + r, r, HxMath.PI, HxMath.PI * 3 / 2);
    ctx.closePath();
  }
  ...
  function rasterInkBox(text:String, fontSize:Float, advance:Float,
    ...
    final pad = Std.int(HxMath.ceil(fontSize)) + 2;
    final w = Std.int(HxMath.ceil(HxMath.max(advance, fontSize * text.length))) + pad * 2;
    final h = pad * 3;
  ...
  static function parseFontSize(fontValue:String):Float {
    for (token in fontValue.split(' ')) {
      if (StringTools.endsWith(token, 'px')) {
        ...
        if (!HxMath.isNaN(size)) return size;
      }
    }
    return 10.0;
  }
  ...
  public function getImageData(x:Float, y:Float, w:Float, h:Float):Dynamic {
    ...
    final data = new flight._internal._UInt8ClampedArray(outWidth * outHeight * 4);
    if (surface != null) {
      surface.flush();
      for (row in 0...outHeight) {
        ...
        for (column in 0...outWidth) {
          ...
          data[dst] = HxMath.round(pixels[src + 2] * scale);
          data[dst + 1] = HxMath.round(pixels[src + 1] * scale);
          data[dst + 2] = HxMath.round(pixels[src] * scale);
          data[dst + 3] = alpha;
        }
      }
    }
...
@:keep
class NativeCanvasGradient implements flight._internal.dom.CanvasGradient {
