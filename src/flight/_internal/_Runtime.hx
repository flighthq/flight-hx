...
class _Runtime {
  ...
  public static function setInterval(callback:Void->Void, delayMs:Float):Dynamic {
    ...
    #else
    final timer = new haxe.Timer(Std.int(HxMath.max(1.0, delayMs)));
    timer.run = callback;
    return timer;
    #end
  }
  ...
  public static function fmod(a:Float, b:Float):Float {
    ...
    #else
    if (b == 0 || HxMath.isNaN(a) || HxMath.isNaN(b) || !HxMath.isFinite(a)) return HxMath.NaN;
    final quotient = a / b;
    final truncated = quotient < 0 ? HxMath.fceil(quotient) : HxMath.ffloor(quotient);
  ...
  public static function toInt32(value:Float):Int {
    ...
    return js.Syntax.code('({0} | 0)', value);
    #else
    if (HxMath.isNaN(value) || !HxMath.isFinite(value)) return 0;
    var truncated = value < 0 ? HxMath.fceil(value) : HxMath.ffloor(value);
