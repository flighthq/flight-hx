// Generated by flight-hx. Do not edit.
package flighthq;

import Math as HxMath;
import flighthq.internal.FlightRuntime;
import flighthq.Image.loadImageResourceFromBytes;
import flighthq.Image.loadImageResourceFromUrl;
import flighthq.Loader.cancelResourceLoad;
import flighthq.Loader.createResourceLoader;
import flighthq.Loader.disposeResourceLoader;
import flighthq.Loader.queueResourceLoad;
import flighthq.Loader.startResourceLoad;
import flighthq.NodeApi.forEachNodeDescendant;
import flighthq.SceneApi.isMesh;
import flighthq.SceneFormats.GltfDocument;
import flighthq.SceneFormats.ObjMaterialLibrary;
import flighthq.SceneFormats.createSceneFrom3ds;
import flighthq.SceneFormats.createSceneFromAwd;
import flighthq.SceneFormats.createSceneFromGlb;
import flighthq.SceneFormats.createSceneFromGltf;
import flighthq.SceneFormats.createSceneFromMd2;
import flighthq.SceneFormats.createSceneFromMd5Mesh;
import flighthq.SceneFormats.createSceneFromObj;
import flighthq.SceneFormats.createScenesFromGlb;
import flighthq.SceneFormats.createScenesFromGltf;
import flighthq.Signals.connectSignal;
import flighthq.Signals.createSignal;
import flighthq.Signals.disconnectSignal;
import flighthq.Signals.emitSignal;
import flighthq.TweenApi.createTween;
import flighthq.Types.EasingFunction;
import flighthq.Types.ExternalSceneResourceRef;
import flighthq.Types.ImageResource;
import flighthq.Types.Kind;
import flighthq.Types.Material;
import flighthq.Types.ResourceLoader;
import flighthq.Types.ResourceResolutionState;
import flighthq.Types.Scene;
import flighthq.Types.SceneNode;
import flighthq.Types.SceneResourceRef;
import flighthq.Types.SceneResourceRefKind;
import flighthq.Types.Signal;
import flighthq.Types.StandardPbrMaterial;
import flighthq.Types.Texture;
import flighthq.Types.TweenManager;
import flighthq.Types.UnlitMaterial;

typedef LoadSceneOptions = { @:optional var resolver:SceneResourceResolver; };

typedef ResolveSceneResourcesOptions = { @:optional var priority:Dynamic; @:optional var select:Dynamic; };

typedef SceneMaterialTextureLister = Dynamic;

typedef SceneMaterialTextureRegistry = { var listers:Dynamic; };

typedef SceneResourceEvent = { var ref:SceneResourceRef; var texture:Texture; };

typedef SceneResourceFetch = Dynamic;

typedef SceneResourceInFlight = { var controller:Dynamic; var key:String; var promise:flighthq.internal.FlightPromise<flighthq.internal.FlightNothing>; };

typedef SceneResourceResolver = { var fetch:SceneResourceFetch; var inFlight:Dynamic; var loader:ResourceLoader; var registry:SceneMaterialTextureRegistry; var signals:Null<SceneResourceSignals>; };

typedef SceneResourceResolverOptions = { @:optional var fetch:SceneResourceFetch; @:optional var maxConcurrent:Float; @:optional var registry:SceneMaterialTextureRegistry; };

typedef SceneResourceRevealOptions = { @:optional var ease:EasingFunction; @:optional var fadeSeconds:Float; @:optional var from:Float; };

typedef SceneResourceSignals = { var onResourceFailed:Signal<Dynamic>; var onResourceResolved:Signal<Dynamic>; };

@:expose("flighthq.SceneResources")
class SceneResources {
  public static final _resolvedVoid__resolveSceneResources:flighthq.internal.FlightPromise<flighthq.internal.FlightNothing> = FlightRuntime.callProperty(FlightRuntime.callProperty(FlightRuntime, 'globalValue', cast (['Promise'] : Array<Dynamic>)), 'resolve', cast ([] : Array<Dynamic>));

  public static function cancelDroppedResolutions__resolveSceneResources(resolver:SceneResourceResolver, working:Dynamic):Void {
    for (__iteration0 in FlightRuntime.iterable(FlightRuntime.field(resolver, 'inFlight'))) {
      var texture:Dynamic = FlightRuntime.getIndex(__iteration0, 0.0);
      var entry:Dynamic = FlightRuntime.getIndex(__iteration0, 1.0);
      if (FlightRuntime.truthy(FlightRuntime.callProperty(working, 'has', cast ([texture] : Array<Dynamic>)))) { continue; }
      FlightRuntime.callProperty(FlightRuntime.field(entry, 'controller'), 'abort', cast ([] : Array<Dynamic>));
      FlightRuntime.callProperty(FlightRuntime.field(resolver, 'inFlight'), 'delete', cast ([texture] : Array<Dynamic>));
      var ref:Dynamic = FlightRuntime.field(texture, 'resource');
      if (FlightRuntime.truthy(FlightRuntime.andValue(!FlightRuntime.looseEquals(ref, null), function():Dynamic return cast FlightRuntime.strictEquals(FlightRuntime.field(ref, 'state'), Types.ResourceResolutionStateValue.Loading)))) {
  FlightRuntime.setField(ref, 'state', Types.ResourceResolutionStateValue.Unresolved);
}
    }
  }

  public static function collectNodeResourceTextures__getSceneResourceTextures(node:SceneNode, registry:SceneMaterialTextureRegistry, out:Array<Texture>, seen:Dynamic, slots:Array<Texture>):Void {
    var materials:Dynamic = cast FlightRuntime.UNDEFINED;
    if (FlightRuntime.truthy(!FlightRuntime.truthy(FlightRuntime.callValue(isMesh, cast ([node] : Array<Dynamic>))))) { return; }
    materials = FlightRuntime.field(node, 'materials');
    {
      var i:Dynamic = 0.0;
      while (FlightRuntime.truthy(FlightRuntime.compare(i, FlightRuntime.field(materials, 'length'), '<'))) {
        var material:Dynamic = (cast FlightRuntime.getIndex(materials, i) : Null<Material>);
        if (FlightRuntime.truthy(FlightRuntime.strictEquals(material, null))) { i++; continue; }
        FlightRuntime.setLength(slots, 0.0);
        FlightRuntime.callValue(getSceneMaterialTextures, cast ([registry, material, slots] : Array<Dynamic>));
        {
          var j:Dynamic = 0.0;
          while (FlightRuntime.truthy(FlightRuntime.compare(j, FlightRuntime.field(slots, 'length'), '<'))) {
            var texture:Dynamic = FlightRuntime.getIndex(slots, j);
            if (FlightRuntime.truthy(FlightRuntime.orValue(FlightRuntime.looseEquals(FlightRuntime.field(texture, 'resource'), null), function():Dynamic return cast FlightRuntime.callProperty(seen, 'has', cast ([texture] : Array<Dynamic>))))) { j++; continue; }
            FlightRuntime.callProperty(seen, 'add', cast ([texture] : Array<Dynamic>));
            FlightRuntime.callProperty(out, 'push', cast ([texture] : Array<Dynamic>));
            j++;
          }
        }
        i++;
      }
    }
  }

  public static function collectPendingTextureOwners__revealSceneResourcesOnResolve(scene:SceneNode, resolver:SceneResourceResolver, out:Dynamic):Void {
    var slots:Array<Texture> = cast FlightRuntime.UNDEFINED;
    var visit:Dynamic = cast FlightRuntime.UNDEFINED;
    slots = cast ([] : Array<Dynamic>);
    visit = function(node:SceneNode) {
  var owner:Dynamic = cast FlightRuntime.UNDEFINED;
  var materials:Dynamic = cast FlightRuntime.UNDEFINED;
  if (FlightRuntime.truthy(!FlightRuntime.truthy(FlightRuntime.callValue(isMesh, cast ([node] : Array<Dynamic>))))) { return; }
  owner = (cast node : SceneNode);
  materials = FlightRuntime.field(node, 'materials');
  {
    var i:Dynamic = 0.0;
    while (FlightRuntime.truthy(FlightRuntime.compare(i, FlightRuntime.field(materials, 'length'), '<'))) {
      var material:Dynamic = (cast FlightRuntime.getIndex(materials, i) : Null<Material>);
      if (FlightRuntime.truthy(FlightRuntime.strictEquals(material, null))) { i++; continue; }
      FlightRuntime.setLength(slots, 0.0);
      FlightRuntime.callValue(getSceneMaterialTextures, cast ([FlightRuntime.field(resolver, 'registry'), material, slots] : Array<Dynamic>));
      {
        var j:Dynamic = 0.0;
        while (FlightRuntime.truthy(FlightRuntime.compare(j, FlightRuntime.field(slots, 'length'), '<'))) {
          var texture:Dynamic = FlightRuntime.getIndex(slots, j);
          if (FlightRuntime.truthy(FlightRuntime.looseEquals(FlightRuntime.field(texture, 'resource'), null))) { j++; continue; }
          var nodes:Dynamic = FlightRuntime.callProperty(out, 'get', cast ([texture] : Array<Dynamic>));
          if (FlightRuntime.truthy(FlightRuntime.strictEquals(nodes, FlightRuntime.UNDEFINED))) {
  (nodes = cast (cast ([] : Array<Dynamic>) : Dynamic));
  FlightRuntime.callProperty(out, 'set', cast ([texture, nodes] : Array<Dynamic>));
}
          if (FlightRuntime.truthy(!FlightRuntime.truthy(FlightRuntime.includes(nodes, owner)))) { FlightRuntime.callProperty(nodes, 'push', cast ([owner] : Array<Dynamic>)); }
          j++;
        }
      }
      i++;
    }
  }
};
    FlightRuntime.callValue(visit, cast ([scene] : Array<Dynamic>));
    FlightRuntime.callValue(forEachNodeDescendant, cast ([scene, function(node:Dynamic) return FlightRuntime.callValue(visit, cast ([(cast node : SceneNode)] : Array<Dynamic>))] : Array<Dynamic>));
  }

  public static function createSceneMaterialTextureRegistry():SceneMaterialTextureRegistry {
    return cast { listers: FlightRuntime.construct(FlightRuntime.callProperty(FlightRuntime, 'globalValue', cast (['Map'] : Array<Dynamic>)), []) };
    return cast null;
  }

  public static function createSceneResourceResolver(?options:SceneResourceResolverOptions):SceneResourceResolver {
    var loader:Dynamic = cast FlightRuntime.UNDEFINED;
    var registry:Dynamic = cast FlightRuntime.UNDEFINED;
    loader = FlightRuntime.callValue(createResourceLoader, cast ([{ dedupe: false, maxConcurrent: FlightRuntime.optionalField(options, 'maxConcurrent'), streaming: true }] : Array<Dynamic>));
    FlightRuntime.callValue(startResourceLoad, cast ([loader] : Array<Dynamic>));
    registry = FlightRuntime.optionalField(options, 'registry');
    if (FlightRuntime.truthy(FlightRuntime.strictEquals(registry, FlightRuntime.UNDEFINED))) {
  (registry = cast (FlightRuntime.callValue(createSceneMaterialTextureRegistry, cast ([] : Array<Dynamic>)) : Dynamic));
  FlightRuntime.callValue(registerBuiltInSceneMaterialTextures, cast ([registry] : Array<Dynamic>));
}
    return cast { fetch: FlightRuntime.coalesce(FlightRuntime.optionalField(options, 'fetch'), function():Dynamic return cast FlightRuntime.callValue(createWebSceneResourceFetch, cast ([] : Array<Dynamic>))), inFlight: FlightRuntime.construct(FlightRuntime.callProperty(FlightRuntime, 'globalValue', cast (['Map'] : Array<Dynamic>)), []), loader: loader, registry: registry, signals: null };
    return cast null;
  }

  public static function createSceneResourceSignals():SceneResourceSignals {
    return cast { onResourceFailed: FlightRuntime.callValue(createSignal, cast ([] : Array<Dynamic>)), onResourceResolved: FlightRuntime.callValue(createSignal, cast ([] : Array<Dynamic>)) };
    return cast null;
  }

  public static function createWebSceneResourceFetch():SceneResourceFetch {
    return cast flighthq.internal.FlightAsync.make(function(ref:Dynamic, signal:Dynamic):flighthq.internal.FlightPromise<Dynamic> {
  var url:Dynamic = cast FlightRuntime.UNDEFINED;
  url = FlightRuntime.callValue(resolveSceneResourceUri, cast ([FlightRuntime.field(ref, 'uri'), FlightRuntime.field(ref, 'basePath')] : Array<Dynamic>));
  try {
  return cast flighthq.internal.FlightAsync.awaitValue(FlightRuntime.callValue(loadImageResourceFromUrl, cast ([url, FlightRuntime.UNDEFINED, signal] : Array<Dynamic>)));
} catch (error:Dynamic) {
  if (FlightRuntime.truthy(FlightRuntime.field(signal, 'aborted'))) { throw error; }
  return cast null;
}
  return cast null;
});
    return cast null;
  }

  public static function disposeSceneResourceResolver(resolver:SceneResourceResolver):Void {
    FlightRuntime.callValue(cancelResourceLoad, cast ([FlightRuntime.field(resolver, 'loader')] : Array<Dynamic>));
    FlightRuntime.callValue(disposeResourceLoader, cast ([FlightRuntime.field(resolver, 'loader')] : Array<Dynamic>));
    for (entry in FlightRuntime.iterable(FlightRuntime.callProperty(FlightRuntime.field(resolver, 'inFlight'), 'values', cast ([] : Array<Dynamic>)))) {
      FlightRuntime.callProperty(FlightRuntime.field(entry, 'controller'), 'abort', cast ([] : Array<Dynamic>));
    }
    FlightRuntime.callProperty(FlightRuntime.field(resolver, 'inFlight'), 'clear', cast ([] : Array<Dynamic>));
  }

  public static function emitSceneResourceEvent__resolveSceneResources(resolver:SceneResourceResolver, texture:Texture, ref:SceneResourceRef, resolved:Bool):Void {
    var signals:Dynamic = cast FlightRuntime.UNDEFINED;
    var event:Dynamic = cast FlightRuntime.UNDEFINED;
    signals = FlightRuntime.field(resolver, 'signals');
    if (FlightRuntime.truthy(FlightRuntime.strictEquals(signals, null))) { return; }
    event = { ref: ref, texture: texture };
    FlightRuntime.callValue(emitSignal, cast ([FlightRuntime.select(resolved, function():Dynamic return cast FlightRuntime.field(signals, 'onResourceResolved'), function():Dynamic return cast FlightRuntime.field(signals, 'onResourceFailed')), event] : Array<Dynamic>));
  }

  public static function enableSceneResourceSignals(resolver:SceneResourceResolver):SceneResourceSignals {
    var signals:Dynamic = cast FlightRuntime.UNDEFINED;
    if (FlightRuntime.truthy(!FlightRuntime.strictEquals(FlightRuntime.field(resolver, 'signals'), null))) { return cast FlightRuntime.field(resolver, 'signals'); }
    signals = FlightRuntime.callValue(createSceneResourceSignals, cast ([] : Array<Dynamic>));
    FlightRuntime.setField(resolver, 'signals', signals);
    return cast signals;
    return cast null;
  }

  public static function failSceneResourceResolution__resolveSceneResources(resolver:SceneResourceResolver, texture:Texture, ref:SceneResourceRef, entry:SceneResourceInFlight):Void {
    if (FlightRuntime.truthy(!FlightRuntime.strictEquals(FlightRuntime.callProperty(FlightRuntime.field(resolver, 'inFlight'), 'get', cast ([texture] : Array<Dynamic>)), entry))) { return; }
    FlightRuntime.callProperty(FlightRuntime.field(resolver, 'inFlight'), 'delete', cast ([texture] : Array<Dynamic>));
    if (FlightRuntime.truthy(FlightRuntime.field(FlightRuntime.field(FlightRuntime.field(entry, 'controller'), 'signal'), 'aborted'))) { return; }
    FlightRuntime.setField(ref, 'state', Types.ResourceResolutionStateValue.Failed);
    FlightRuntime.callValue(SceneResources.emitSceneResourceEvent__resolveSceneResources, cast ([resolver, texture, ref, false] : Array<Dynamic>));
  }

  public static function finishSceneResourceResolution__resolveSceneResources(resolver:SceneResourceResolver, texture:Texture, ref:SceneResourceRef, entry:SceneResourceInFlight, image:Null<ImageResource>):Void {
    if (FlightRuntime.truthy(!FlightRuntime.strictEquals(FlightRuntime.callProperty(FlightRuntime.field(resolver, 'inFlight'), 'get', cast ([texture] : Array<Dynamic>)), entry))) { return; }
    FlightRuntime.callProperty(FlightRuntime.field(resolver, 'inFlight'), 'delete', cast ([texture] : Array<Dynamic>));
    if (FlightRuntime.truthy(FlightRuntime.strictEquals(image, null))) {
  FlightRuntime.setField(ref, 'state', Types.ResourceResolutionStateValue.Failed);
  FlightRuntime.callValue(SceneResources.emitSceneResourceEvent__resolveSceneResources, cast ([resolver, texture, ref, false] : Array<Dynamic>));
  return;
}
    FlightRuntime.setField(texture, 'image', image);
    FlightRuntime.setField(ref, 'state', Types.ResourceResolutionStateValue.Resolved);
    FlightRuntime.callValue(SceneResources.emitSceneResourceEvent__resolveSceneResources, cast ([resolver, texture, ref, true] : Array<Dynamic>));
  }

  public static function getSceneMaterialTextures(registry:SceneMaterialTextureRegistry, material:Material, out:Array<Texture>):Void {
    var lister:Dynamic = cast FlightRuntime.UNDEFINED;
    lister = FlightRuntime.callProperty(FlightRuntime.field(registry, 'listers'), 'get', cast ([FlightRuntime.field(material, 'kind')] : Array<Dynamic>));
    if (FlightRuntime.truthy(!FlightRuntime.strictEquals(lister, FlightRuntime.UNDEFINED))) { FlightRuntime.callValue(lister, cast ([material, out] : Array<Dynamic>)); }
  }

  public static function getSceneResourceSignals(resolver:SceneResourceResolver):Null<SceneResourceSignals> {
    return cast FlightRuntime.field(resolver, 'signals');
    return cast null;
  }

  public static function getSceneResourceTextures(scene:SceneNode, registry:SceneMaterialTextureRegistry, out:Array<Texture>):Void {
    var seen:Dynamic = cast FlightRuntime.UNDEFINED;
    var slots:Array<Texture> = cast FlightRuntime.UNDEFINED;
    FlightRuntime.setLength(out, 0.0);
    seen = FlightRuntime.construct(FlightRuntime.callProperty(FlightRuntime, 'globalValue', cast (['Set'] : Array<Dynamic>)), []);
    slots = cast ([] : Array<Dynamic>);
    FlightRuntime.callValue(SceneResources.collectNodeResourceTextures__getSceneResourceTextures, cast ([scene, registry, out, seen, slots] : Array<Dynamic>));
    FlightRuntime.callValue(forEachNodeDescendant, cast ([scene, function(node:Dynamic) return FlightRuntime.callValue(SceneResources.collectNodeResourceTextures__getSceneResourceTextures, cast ([(cast node : SceneNode), registry, out, seen, slots] : Array<Dynamic>))] : Array<Dynamic>));
  }

  public static function isAbsoluteSceneResourceUri__sceneResourceFetch(uri:String):Bool {
    return cast FlightRuntime.orValue(StringTools.startsWith(uri, '/'), function():Dynamic return cast FlightRuntime.callProperty(FlightRuntime.regexp('^[a-zA-Z][a-zA-Z0-9+.-]*:', ''), 'test', cast ([uri] : Array<Dynamic>)));
    return cast null;
  }

  public static function listStandardPbrMaterialTextures__sceneMaterialTextureRegistry(material:Material, out:Array<Texture>):Void {
    var pbr:Dynamic = cast FlightRuntime.UNDEFINED;
    pbr = (cast material : StandardPbrMaterial);
    if (FlightRuntime.truthy(!FlightRuntime.strictEquals(FlightRuntime.field(pbr, 'baseColorMap'), null))) { FlightRuntime.callProperty(out, 'push', cast ([FlightRuntime.field(pbr, 'baseColorMap')] : Array<Dynamic>)); }
    if (FlightRuntime.truthy(!FlightRuntime.strictEquals(FlightRuntime.field(pbr, 'emissiveMap'), null))) { FlightRuntime.callProperty(out, 'push', cast ([FlightRuntime.field(pbr, 'emissiveMap')] : Array<Dynamic>)); }
    if (FlightRuntime.truthy(!FlightRuntime.strictEquals(FlightRuntime.field(pbr, 'metallicRoughnessMap'), null))) { FlightRuntime.callProperty(out, 'push', cast ([FlightRuntime.field(pbr, 'metallicRoughnessMap')] : Array<Dynamic>)); }
    if (FlightRuntime.truthy(!FlightRuntime.strictEquals(FlightRuntime.field(pbr, 'normalMap'), null))) { FlightRuntime.callProperty(out, 'push', cast ([FlightRuntime.field(pbr, 'normalMap')] : Array<Dynamic>)); }
    if (FlightRuntime.truthy(!FlightRuntime.strictEquals(FlightRuntime.field(pbr, 'occlusionMap'), null))) { FlightRuntime.callProperty(out, 'push', cast ([FlightRuntime.field(pbr, 'occlusionMap')] : Array<Dynamic>)); }
  }

  public static function listUnlitMaterialTextures__sceneMaterialTextureRegistry(material:Material, out:Array<Texture>):Void {
    var unlit:Dynamic = cast FlightRuntime.UNDEFINED;
    unlit = (cast material : UnlitMaterial);
    if (FlightRuntime.truthy(!FlightRuntime.strictEquals(FlightRuntime.field(unlit, 'baseColorMap'), null))) { FlightRuntime.callProperty(out, 'push', cast ([FlightRuntime.field(unlit, 'baseColorMap')] : Array<Dynamic>)); }
  }

  public static function loadSceneFrom3ds(bytes:Dynamic, ?options:LoadSceneOptions):flighthq.internal.FlightPromise<Scene> {
    return cast flighthq.internal.FlightAsync.make(function():flighthq.internal.FlightPromise<Scene> {
      var scene:Dynamic = cast FlightRuntime.UNDEFINED;
      scene = FlightRuntime.callValue(createSceneFrom3ds, cast ([bytes] : Array<Dynamic>));
      flighthq.internal.FlightAsync.awaitValue(FlightRuntime.callValue(resolveScenesWithOptions, cast ([cast ([scene] : Array<Dynamic>), options] : Array<Dynamic>)));
      return cast scene;
      return cast null;
    })();
  }

  public static function loadSceneFromAwd(bytes:Dynamic, ?options:LoadSceneOptions):flighthq.internal.FlightPromise<Scene> {
    return cast flighthq.internal.FlightAsync.make(function():flighthq.internal.FlightPromise<Scene> {
      var scene:Dynamic = cast FlightRuntime.UNDEFINED;
      scene = FlightRuntime.callValue(createSceneFromAwd, cast ([bytes] : Array<Dynamic>));
      flighthq.internal.FlightAsync.awaitValue(FlightRuntime.callValue(resolveScenesWithOptions, cast ([cast ([scene] : Array<Dynamic>), options] : Array<Dynamic>)));
      return cast scene;
      return cast null;
    })();
  }

  public static function loadSceneFromGlb(bytes:Dynamic, ?options:LoadSceneOptions):flighthq.internal.FlightPromise<Scene> {
    return cast flighthq.internal.FlightAsync.make(function():flighthq.internal.FlightPromise<Scene> {
      var scene:Dynamic = cast FlightRuntime.UNDEFINED;
      scene = FlightRuntime.callValue(createSceneFromGlb, cast ([bytes] : Array<Dynamic>));
      flighthq.internal.FlightAsync.awaitValue(FlightRuntime.callValue(resolveScenesWithOptions, cast ([cast ([scene] : Array<Dynamic>), options] : Array<Dynamic>)));
      return cast scene;
      return cast null;
    })();
  }

  public static function loadSceneFromGltf(source:Dynamic, ?options:LoadSceneOptions):flighthq.internal.FlightPromise<Scene> {
    return cast flighthq.internal.FlightAsync.make(function():flighthq.internal.FlightPromise<Scene> {
      var scene:Dynamic = cast FlightRuntime.UNDEFINED;
      scene = FlightRuntime.callValue(createSceneFromGltf, cast ([source] : Array<Dynamic>));
      flighthq.internal.FlightAsync.awaitValue(FlightRuntime.callValue(resolveScenesWithOptions, cast ([cast ([scene] : Array<Dynamic>), options] : Array<Dynamic>)));
      return cast scene;
      return cast null;
    })();
  }

  public static function loadSceneFromMd2(bytes:Dynamic, ?options:LoadSceneOptions):flighthq.internal.FlightPromise<Scene> {
    return cast flighthq.internal.FlightAsync.make(function():flighthq.internal.FlightPromise<Scene> {
      var scene:Dynamic = cast FlightRuntime.UNDEFINED;
      scene = FlightRuntime.callValue(createSceneFromMd2, cast ([bytes] : Array<Dynamic>));
      flighthq.internal.FlightAsync.awaitValue(FlightRuntime.callValue(resolveScenesWithOptions, cast ([cast ([scene] : Array<Dynamic>), options] : Array<Dynamic>)));
      return cast scene;
      return cast null;
    })();
  }

  public static function loadSceneFromMd5Mesh(meshSource:String, ?options:LoadSceneOptions):flighthq.internal.FlightPromise<Scene> {
    return cast flighthq.internal.FlightAsync.make(function():flighthq.internal.FlightPromise<Scene> {
      var scene:Dynamic = cast FlightRuntime.UNDEFINED;
      scene = FlightRuntime.callValue(createSceneFromMd5Mesh, cast ([meshSource] : Array<Dynamic>));
      flighthq.internal.FlightAsync.awaitValue(FlightRuntime.callValue(resolveScenesWithOptions, cast ([cast ([scene] : Array<Dynamic>), options] : Array<Dynamic>)));
      return cast scene;
      return cast null;
    })();
  }

  public static function loadSceneFromObj(source:String, ?materials:ObjMaterialLibrary, ?options:LoadSceneOptions):flighthq.internal.FlightPromise<Scene> {
    return cast flighthq.internal.FlightAsync.make(function():flighthq.internal.FlightPromise<Scene> {
      var scene:Dynamic = cast FlightRuntime.UNDEFINED;
      scene = FlightRuntime.callValue(createSceneFromObj, cast ([source, materials] : Array<Dynamic>));
      flighthq.internal.FlightAsync.awaitValue(FlightRuntime.callValue(resolveScenesWithOptions, cast ([cast ([scene] : Array<Dynamic>), options] : Array<Dynamic>)));
      return cast scene;
      return cast null;
    })();
  }

  public static function loadScenesFromGlb(bytes:Dynamic, ?options:LoadSceneOptions):flighthq.internal.FlightPromise<Array<Scene>> {
    return cast flighthq.internal.FlightAsync.make(function():flighthq.internal.FlightPromise<Array<Scene>> {
      var scenes:Dynamic = cast FlightRuntime.UNDEFINED;
      scenes = FlightRuntime.callValue(createScenesFromGlb, cast ([bytes] : Array<Dynamic>));
      flighthq.internal.FlightAsync.awaitValue(FlightRuntime.callValue(resolveScenesWithOptions, cast ([scenes, options] : Array<Dynamic>)));
      return cast scenes;
      return cast null;
    })();
  }

  public static function loadScenesFromGltf(source:Dynamic, ?options:LoadSceneOptions):flighthq.internal.FlightPromise<Array<Scene>> {
    return cast flighthq.internal.FlightAsync.make(function():flighthq.internal.FlightPromise<Array<Scene>> {
      var scenes:Dynamic = cast FlightRuntime.UNDEFINED;
      scenes = FlightRuntime.callValue(createScenesFromGltf, cast ([source] : Array<Dynamic>));
      flighthq.internal.FlightAsync.awaitValue(FlightRuntime.callValue(resolveScenesWithOptions, cast ([scenes, options] : Array<Dynamic>)));
      return cast scenes;
      return cast null;
    })();
  }

  public static function registerBuiltInSceneMaterialTextures(registry:SceneMaterialTextureRegistry):Void {
    FlightRuntime.callValue(registerSceneMaterialTextures, cast ([registry, Types.StandardPbrMaterialKind, SceneResources.listStandardPbrMaterialTextures__sceneMaterialTextureRegistry] : Array<Dynamic>));
    FlightRuntime.callValue(registerSceneMaterialTextures, cast ([registry, Types.UnlitMaterialKind, SceneResources.listUnlitMaterialTextures__sceneMaterialTextureRegistry] : Array<Dynamic>));
  }

  public static function registerSceneMaterialTextures(registry:SceneMaterialTextureRegistry, kind:Kind, lister:SceneMaterialTextureLister):Void {
    FlightRuntime.callProperty(FlightRuntime.field(registry, 'listers'), 'set', cast ([kind, lister] : Array<Dynamic>));
  }

  public static function requestWorkingResolutions__resolveSceneResources(resolver:SceneResourceResolver, working:Dynamic, ?options:ResolveSceneResourcesOptions):Void {
    for (texture in FlightRuntime.iterable(working)) {
      var ref:Dynamic = FlightRuntime.field(texture, 'resource');
      if (FlightRuntime.truthy(FlightRuntime.orValue(FlightRuntime.orValue(FlightRuntime.looseEquals(ref, null), function():Dynamic return cast FlightRuntime.callProperty(FlightRuntime.field(resolver, 'inFlight'), 'has', cast ([texture] : Array<Dynamic>))), function():Dynamic return cast !FlightRuntime.strictEquals(FlightRuntime.field(ref, 'state'), Types.ResourceResolutionStateValue.Unresolved)))) {
  continue;
}
      FlightRuntime.setField(ref, 'state', Types.ResourceResolutionStateValue.Loading);
      var controller:Dynamic = FlightRuntime.construct(FlightRuntime.callProperty(FlightRuntime, 'globalValue', cast (['AbortController'] : Array<Dynamic>)), []);
      var priority:Dynamic = FlightRuntime.coalesce(FlightRuntime.callOptionalProperty(options, 'priority', cast ([texture, ref] : Array<Dynamic>)), function():Dynamic return cast 0.0);
      var handle:Dynamic = FlightRuntime.callValue(queueResourceLoad, cast ([FlightRuntime.field(resolver, 'loader'), { load: function(loaderSignal:Dynamic) {
  if (FlightRuntime.truthy(FlightRuntime.field(loaderSignal, 'aborted'))) { FlightRuntime.callProperty(controller, 'abort', cast ([FlightRuntime.field(loaderSignal, 'reason')] : Array<Dynamic>)); } else { FlightRuntime.callProperty(loaderSignal, 'addEventListener', cast (['abort', function() return FlightRuntime.callProperty(controller, 'abort', cast ([FlightRuntime.field(loaderSignal, 'reason')] : Array<Dynamic>)), { once: true }] : Array<Dynamic>)); }
  return cast FlightRuntime.callValue(resolveOneSceneResourceTexture, cast ([resolver, ref, FlightRuntime.field(controller, 'signal')] : Array<Dynamic>));
}, priority: priority }] : Array<Dynamic>));
      var entry:SceneResourceInFlight = { controller: controller, key: FlightRuntime.field(handle, 'key'), promise: SceneResources._resolvedVoid__resolveSceneResources };
      FlightRuntime.setField(entry, 'promise', FlightRuntime.callProperty(FlightRuntime.field(handle, 'promise'), 'then', cast ([function(image:Dynamic) return FlightRuntime.callValue(SceneResources.finishSceneResourceResolution__resolveSceneResources, cast ([resolver, texture, ref, entry, image] : Array<Dynamic>)), function() return FlightRuntime.callValue(SceneResources.failSceneResourceResolution__resolveSceneResources, cast ([resolver, texture, ref, entry] : Array<Dynamic>))] : Array<Dynamic>)));
      FlightRuntime.callProperty(FlightRuntime.field(resolver, 'inFlight'), 'set', cast ([texture, entry] : Array<Dynamic>));
    }
  }

  public static function resolveOneSceneResourceTexture(resolver:SceneResourceResolver, ref:SceneResourceRef, signal:Dynamic):flighthq.internal.FlightPromise<Null<ImageResource>> {
    if (FlightRuntime.truthy(FlightRuntime.strictEquals(FlightRuntime.field(ref, 'kind'), Types.SceneResourceRefKindValue.Embedded))) {
  return cast FlightRuntime.callValue(loadImageResourceFromBytes, cast ([FlightRuntime.field(ref, 'bytes'), FlightRuntime.coalesce(FlightRuntime.field(ref, 'mimeType'), function():Dynamic return cast FlightRuntime.UNDEFINED), signal] : Array<Dynamic>));
}
    return cast FlightRuntime.callProperty(resolver, 'fetch', cast ([ref, signal] : Array<Dynamic>));
    return cast null;
  }

  public static function resolveSceneResources(scene:SceneNode, resolver:SceneResourceResolver, ?options:ResolveSceneResourcesOptions):Void {
    var pending:Array<Texture> = cast FlightRuntime.UNDEFINED;
    var working:Dynamic = cast FlightRuntime.UNDEFINED;
    pending = cast ([] : Array<Dynamic>);
    FlightRuntime.callValue(getSceneResourceTextures, cast ([scene, FlightRuntime.field(resolver, 'registry'), pending] : Array<Dynamic>));
    working = FlightRuntime.construct(FlightRuntime.callProperty(FlightRuntime, 'globalValue', cast (['Set'] : Array<Dynamic>)), []);
    {
      var i:Dynamic = 0.0;
      while (FlightRuntime.truthy(FlightRuntime.compare(i, FlightRuntime.field(pending, 'length'), '<'))) {
        var texture:Dynamic = FlightRuntime.getIndex(pending, i);
        var ref:Dynamic = FlightRuntime.field(texture, 'resource');
        if (FlightRuntime.truthy(FlightRuntime.looseEquals(ref, null))) { i++; continue; }
        if (FlightRuntime.truthy(FlightRuntime.orValue(FlightRuntime.strictEquals(FlightRuntime.optionalField(options, 'select'), FlightRuntime.UNDEFINED), function():Dynamic return cast FlightRuntime.callProperty(options, 'select', cast ([texture, ref] : Array<Dynamic>))))) { FlightRuntime.callProperty(working, 'add', cast ([texture] : Array<Dynamic>)); }
        i++;
      }
    }
    FlightRuntime.callValue(SceneResources.cancelDroppedResolutions__resolveSceneResources, cast ([resolver, working] : Array<Dynamic>));
    FlightRuntime.callValue(SceneResources.requestWorkingResolutions__resolveSceneResources, cast ([resolver, working, options] : Array<Dynamic>));
  }

  public static function resolveSceneResourcesAndWait(scene:SceneNode, resolver:SceneResourceResolver, ?options:ResolveSceneResourcesOptions):flighthq.internal.FlightPromise<flighthq.internal.FlightNothing> {
    return cast flighthq.internal.FlightAsync.make(function():flighthq.internal.FlightPromise<flighthq.internal.FlightNothing> {
      var promises:Array<flighthq.internal.FlightPromise<flighthq.internal.FlightNothing>> = cast FlightRuntime.UNDEFINED;
      FlightRuntime.callValue(resolveSceneResources, cast ([scene, resolver, options] : Array<Dynamic>));
      promises = cast ([] : Array<Dynamic>);
      for (entry in FlightRuntime.iterable(FlightRuntime.callProperty(FlightRuntime.field(resolver, 'inFlight'), 'values', cast ([] : Array<Dynamic>)))) {
        FlightRuntime.callProperty(promises, 'push', cast ([FlightRuntime.field(entry, 'promise')] : Array<Dynamic>));
      }
      flighthq.internal.FlightAsync.awaitValue(FlightRuntime.callProperty(FlightRuntime.callProperty(FlightRuntime, 'globalValue', cast (['Promise'] : Array<Dynamic>)), 'allSettled', cast ([promises] : Array<Dynamic>)));
      #if js
      return;
      #else
      return cast null;
      #end
    })();
  }

  public static function resolveSceneResourceUri(uri:String, basePath:Null<String>):String {
    if (FlightRuntime.truthy(FlightRuntime.orValue(FlightRuntime.strictEquals(basePath, null), function():Dynamic return cast FlightRuntime.callValue(SceneResources.isAbsoluteSceneResourceUri__sceneResourceFetch, cast ([uri] : Array<Dynamic>))))) { return cast uri; }
    if (FlightRuntime.truthy(FlightRuntime.orValue(StringTools.endsWith(Std.string(basePath), '/'), function():Dynamic return cast StringTools.startsWith(uri, '/')))) { return cast '' + Std.string(basePath) + '' + Std.string(uri) + ''; }
    return cast '' + Std.string(basePath) + '/' + Std.string(uri) + '';
    return cast null;
  }

  public static function resolveScenesWithOptions(scenes:Array<Scene>, ?options:LoadSceneOptions):flighthq.internal.FlightPromise<flighthq.internal.FlightNothing> {
    return cast flighthq.internal.FlightAsync.make(function():flighthq.internal.FlightPromise<flighthq.internal.FlightNothing> {
      var resolver:Dynamic = cast FlightRuntime.UNDEFINED;
      resolver = FlightRuntime.coalesce(FlightRuntime.optionalField(options, 'resolver'), function():Dynamic return cast FlightRuntime.callValue(createSceneResourceResolver, cast ([] : Array<Dynamic>)));
      try {
        try {
  for (scene in FlightRuntime.iterable(scenes)) {
    flighthq.internal.FlightAsync.awaitValue(FlightRuntime.callValue(resolveSceneResourcesAndWait, cast ([FlightRuntime.field(scene, 'root'), resolver] : Array<Dynamic>)));
  }
} catch (__error:Dynamic) { throw __error; }
      } catch (__finallyError10:Dynamic) {
        {
          if (FlightRuntime.truthy(FlightRuntime.strictEquals(FlightRuntime.optionalField(options, 'resolver'), FlightRuntime.UNDEFINED))) { FlightRuntime.callValue(disposeSceneResourceResolver, cast ([resolver] : Array<Dynamic>)); }
        }
        throw __finallyError10;
      }
      {
        if (FlightRuntime.truthy(FlightRuntime.strictEquals(FlightRuntime.optionalField(options, 'resolver'), FlightRuntime.UNDEFINED))) { FlightRuntime.callValue(disposeSceneResourceResolver, cast ([resolver] : Array<Dynamic>)); }
      }
      #if js
      return;
      #else
      return cast null;
      #end
    })();
  }

  public static function revealSceneResourcesOnResolve(resolver:SceneResourceResolver, scene:SceneNode, tweenManager:TweenManager, ?options:SceneResourceRevealOptions):Dynamic {
    var fadeSeconds:Dynamic = cast FlightRuntime.UNDEFINED;
    var from:Dynamic = cast FlightRuntime.UNDEFINED;
    var tweenOptions:Dynamic = cast FlightRuntime.UNDEFINED;
    var owners:Dynamic = cast FlightRuntime.UNDEFINED;
    var signals:Dynamic = cast FlightRuntime.UNDEFINED;
    var slot:Dynamic = cast FlightRuntime.UNDEFINED;
    fadeSeconds = FlightRuntime.coalesce(FlightRuntime.optionalField(options, 'fadeSeconds'), function():Dynamic return cast 0.4);
    from = FlightRuntime.coalesce(FlightRuntime.optionalField(options, 'from'), function():Dynamic return cast 0.0);
    tweenOptions = FlightRuntime.select(!FlightRuntime.strictEquals(FlightRuntime.optionalField(options, 'ease'), FlightRuntime.UNDEFINED), function():Dynamic return cast { ease: FlightRuntime.field(options, 'ease') }, function():Dynamic return cast FlightRuntime.UNDEFINED);
    owners = FlightRuntime.construct(FlightRuntime.callProperty(FlightRuntime, 'globalValue', cast (['Map'] : Array<Dynamic>)), []);
    FlightRuntime.callValue(SceneResources.collectPendingTextureOwners__revealSceneResourcesOnResolve, cast ([scene, resolver, owners] : Array<Dynamic>));
    for (nodes in FlightRuntime.iterable(FlightRuntime.callProperty(owners, 'values', cast ([] : Array<Dynamic>)))) {
      for (node in FlightRuntime.iterable(nodes)) {
        FlightRuntime.setField(node, 'alpha', from);
      }
    }
    signals = FlightRuntime.callValue(enableSceneResourceSignals, cast ([resolver] : Array<Dynamic>));
    slot = function(event:{ var texture:Texture; }) {
  var nodes:Dynamic = cast FlightRuntime.UNDEFINED;
  nodes = FlightRuntime.callProperty(owners, 'get', cast ([FlightRuntime.field(event, 'texture')] : Array<Dynamic>));
  if (FlightRuntime.truthy(FlightRuntime.strictEquals(nodes, FlightRuntime.UNDEFINED))) { return; }
  for (node in FlightRuntime.iterable(nodes)) {
    FlightRuntime.callValue(createTween, cast ([tweenManager, node, fadeSeconds, { alpha: 1.0 }, tweenOptions] : Array<Dynamic>));
  }
};
    FlightRuntime.callValue(connectSignal, cast ([FlightRuntime.field(signals, 'onResourceResolved'), slot] : Array<Dynamic>));
    return cast function() return FlightRuntime.callValue(disconnectSignal, cast ([FlightRuntime.field(signals, 'onResourceResolved'), slot] : Array<Dynamic>));
    return cast null;
  }
}
