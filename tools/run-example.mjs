...
// Compile and run a Haxe example from `examples/<name>` against the maintained
// and generated classpaths, on any supported Haxe target. Examples are written directly
...
if (!name || name.startsWith('-')) {
  const available = existsSync(examplesDirectory)
    ...
    : [];
  process.stderr.write(`Usage: node tools/run-example.mjs <name> [${supported.join('|')}]\n`);
...
const common = ['-cp', 'src', '-cp', 'generated', '-cp', exampleDirectory, '--main', 'Main'];
