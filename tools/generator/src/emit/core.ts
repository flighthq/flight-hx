...
const packageTestMocksCache = new Map<string, Map<string, MockedSpecifier>>();
...
function declarationRuntimePath(module: IrModule, declaration: IrDeclaration): string {
  return `${module.haxePackage ?? 'flight'}.${declaration.name}`;
}
      ...
      return {
        ...
        runtimePath:
          declaration.kind === 'class' && owner
            ? declarationRuntimePath(owner, declaration)
...
function emitJavaScriptSourceBridge(
  ...
  const mockedSpecifiers = collectPackageTestMocks(file);
  ...
  for (const statement of source.statements) {
    ...
    for (const element of statement.exportClause.elements) {
      ...
      if (!imported) {
        if (!sourceSpecifier) {
          const publicExport = resolvePackageExportLane(inventoryByName, packageName).exports.find(
            (record) => record.name === publicName && record.runtime,
          );
          if (publicExport) {
            reexports.push(
              `export const ${publicName} = compiled.flight.${packageNameToModule(packageName)}.${publicName};`,
            );
          }
        }
        continue;
  ...
  const needsCompiled =
    ...
    sourceDeclarations.some((declaration) => declaration.kind === 'class') ||
    reexports.some((reexport) => reexport.includes('compiled.'));
  const requiresCallSync = dependencies.length > 0;
  const bridgedFunctions = new Map(
    sourceDeclarations
      .filter((declaration) => declaration.kind === 'function' && requiresCallSync)
      .map((declaration) => [declaration.name, publicName(declaration)]),
  );
  const identityAssignments = sourceDeclarations.flatMap((declaration) => {
    if (declaration.kind !== 'variable' || declaration.initializer?.kind !== 'object') return [];
    return declaration.initializer.properties.flatMap((property) => {
      if (property.kind !== 'property' || property.value.kind !== 'identifier') return [];
      const bridgedName = bridgedFunctions.get(property.value.name);
      return bridgedName ? [`api.${declaration.name}[${JSON.stringify(property.name)}] = ${bridgedName};`] : [];
    });
  });
  return [
    '// Generated by flight-hx. Do not edit.',
    ...
    ...sourceDeclarations.map((declaration) => {
      ...
      if (declaration.kind === 'class') {
        ...
        return `export const ${exportedName} = compiled.${owner ? declarationRuntimePath(owner, declaration) : `flight.${declaration.name}`};`;
      }
      ...
      if (declaration.kind === 'function') {
        ...
        if (!requiresCallSync && !syncsMutableExports) {
        ...
        return [
          ...
          `export function ${exportedName}(...args) {${requiresCallSync ? ' __syncDependencies();' : ''} const result = ${implementationName}(...args);${syncsMutableExports ? ' __syncMutableExports();' : ''} return result; }`,
          `api.${declaration.name} = ${exportedName};`,
          `compiled.flight.${packageNameToModule(packageName)}.${exportedName} = ${exportedName};`,
        ].join('\n');
      }
      if (declaration.kind === 'variable') {
        if (requiresCallSync && !declaration.mutable && declaration.initializer?.kind === 'function') {
          const implementationName = `__bridgeImplementation_${declaration.name}`;
          return [
            `const ${implementationName} = api.${declaration.name};`,
...
function collectPackageTestMocks(sourceFile: string): Map<string, MockedSpecifier> {
  const directory = path.dirname(sourceFile);
  const cached = packageTestMocksCache.get(directory);
  if (cached) return cached;
  const combined = new Map<string, MockedSpecifier>();
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    if (!entry.isFile() || !entry.name.endsWith('.test.ts')) continue;
    for (const [specifier, policy] of collectTestMocks(path.join(directory, entry.name))) {
      const target = combined.get(specifier) ?? { allExports: false, exports: new Set<string>() };
      target.allExports ||= policy.allExports;
      for (const name of policy.exports) target.exports.add(name);
      combined.set(specifier, target);
    }
  }
  packageTestMocksCache.set(directory, combined);
  return combined;
}

function collectTestMocks(testFile: string): Map<string, MockedSpecifier> {
  const source = ts.createSourceFile(
    testFile,
