...
describe('typed struct stable declaration identity', () => {
  it('locks the previous report through the approved migration dispositions', () => {
    ...
    expect(discovery.candidates).toHaveLength(1_940);
  ...
  it('admits only new rows as audit-only and records reviewed replacements separately', () => {
    ...
    expect(newlyDiscovered).toHaveLength(1_538);
...
describe('typed struct analysis', () => {
  ...
  it('re-proves the reviewed cpp controls against the complete checker-derived universe', () => {
    ...
    expect(report.summary).toMatchObject({
      auditOnlySchemas: 1_538,
      bindableAccesses: 28_027,
      candidates: 1_940,
      directAccesses: 10_740,
      directSchemas: 397,
      eligible: 1_473,
      escapes: 10_948,
      fields: 22_915,
      ineligible: 467,
      pendingAccesses: 17_287,
      reflectiveSurvivors: 440,
    });
    ...
    expect(classAudit.summary.schemas).toBe(1_473);
    expect(provenance.summary).toMatchObject({
      candidateSchemas: 715,
      closedSchemas: 535,
      containmentEdges: 1_748,
    });
    ...
    expect(report.summary.directAccesses).toBe(10_740);
