...
describe('public Haxe facades', () => {
  it('emits the broad SDK facade and renamed package re-exports', () => {
    ...
    const sdk = readFileSync(path.join(workspace, 'src', 'flight', 'Sdk.hx'), 'utf8');
    const displayObjectGl = readFileSync(path.join(workspace, 'src', 'flight', 'DisplayObjectGl.hx'), 'utf8');
