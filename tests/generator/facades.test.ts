...
describe('public Haxe facades', () => {
  it('emits the broad SDK facade and renamed package re-exports', () => {
    ...
    const sdk = readFileSync(path.join(workspace, 'src', 'flighthq', 'Sdk.hx'), 'utf8');
    const displayObjectGl = readFileSync(path.join(workspace, 'src', 'flighthq', 'DisplayObjectGl.hx'), 'utf8');
