    ...
    case 'forOf': {
      ...
      const lines = statement.async
        ? [
            ...
            '  while (true) {',
            `    var ${step}:Dynamic = flight.internal.FlightAsync.awaitValue(FlightRuntime.callProperty(${iterator}, 'next', cast ([] : Array<Dynamic>)));`,
...
function isPromiseNothingType(type: IrType): boolean {
  return (
    type.kind === 'named' &&
    type.name === 'flight.internal.FlightPromise' &&
    type.arguments[0]?.kind === 'named' &&
    type.arguments[0].name === 'flight.internal.FlightNothing'
  );
}
...
function emitExpression(expression: IrExpression): string {
  switch (expression.kind) {
    ...
    case 'await':
      return `flight.internal.FlightAsync.awaitValue(${emitExpression(expression.expression)})`;
    case 'assignment':
    case 'binary': {
    ...
    case 'function': {
      ...
      if (expression.expression) {
        if (expression.async) {
          ...
          return `flight.internal.FlightAsync.make(${output})`;
        }
      ...
      const output = `function${name}(${parameters})${returns} {\n${body}\n}`;
      return expression.async ? `flight.internal.FlightAsync.make(${output})` : output;
    }
    case 'identifier':
