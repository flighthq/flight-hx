...
export function generateCoreModules(workspaceDirectory: string, check: boolean): CoreGenerationReport {
  ...
  for (const item of loweredPackages) {
    ...
    item.lowered.imports = [
      ...new Set([
        ...item.lowered.imports.map((importPath) => {
          const match = /^flight\.[^.]+\.([^ ]+)(.*)$/u.exec(importPath);
          ...
          if (
            ...
            return `flight.Types.${valueAlias} as ${localAlias}`;
          }
          return match?.[1] && canonicalImportNames.has(match[1])
            ? `flight.Types.${match[1]}${match[2] ?? ''}`
            : importPath;
        }),
        ...(item.packageName === '@flighthq/types'
          ? []
          : [...referencedCanonicalTypes].map((name) => `flight.Types.${name}`)),
      ]),
    ].sort();
    if (item.packageName !== '@flighthq/types') {
      ...
      item.lowered.imports = item.lowered.imports.filter((importPath) => {
        const match = /^flight\.Types\.([^ ]+)/u.exec(importPath);
        return (
          !match?.[1] ||
  ...
  const generatedDirectory = path.join(workspaceDirectory, 'src', 'flight');
  ...
  mkdirSync(path.join(generatedDirectory, 'internal'), { recursive: true });
  for (const runtimeFile of [
    'FlightAsync.hx',
    'FlightFloat32Array.hx',
    'FlightInt16Array.hx',
    'FlightMap.hx',
    'FlightNothing.hx',
    'FlightPromise.hx',
    'FlightRuntime.hx',
    'FlightSet.hx',
    'FlightUInt16Array.hx',
  ]) {
    const runtimeSource = readFileSync(path.join(workspaceDirectory, 'templates', 'runtime', runtimeFile), 'utf8');
    writeOrCheck(path.join(generatedDirectory, 'internal', runtimeFile), runtimeSource, check);
  }
...
function buildPublicFacades(
  ...
  const addFacade = (
    ...
    target.imports.push(`flight.${originModule.name}.${origin.name} as ${alias}`);
  ...
  for (const target of modules) {
    ...
    for (const typeName of referencedTypes) {
      ...
      if (owner && owner !== target) target.imports.push(`flight.${owner.name}.${typeName}`);
    }
    target.imports = [...new Set(target.imports)].sort();
  }
}
...
function emitJavaScriptBridge(
  ...
  return [
    ...
    '',
    ...(exports.length > 0 ? [`const api = compiled.flight.${moduleName};`] : ['void compiled;']),
    ...exports.map(({ generatedName, kind, mutable, publicName }) =>
      kind === 'class'
        ? `export const ${publicName} = compiled.flight.${generatedName};`
        : kind === 'enum'
...
function emitJavaScriptSourceBridge(
  ...
  return [
    ...
    '',
    ...(needsApi ? [`const api = compiled.flight.${moduleName};`] : []),
    ...(dependencies.length > 0
      ? [
          'function __syncDependencies() {',
          ...dependencies.map(
            (dependency) =>
              `  compiled.flight.${dependency.moduleName}.${dependency.generatedName} = ${dependency.specifier}.${dependency.importedName};`,
          ),
          '}',
        ]
      : ['function __syncDependencies() {}']),
    ...
    ...sourceDeclarations.map((declaration) => {
      ...
      if (declaration.kind === 'class') return `export const ${exportedName} = compiled.flight.${declaration.name};`;
...
function collectHaxeImports(source: ts.SourceFile, imports: Set<string>, packageName: string): void {
  for (const statement of source.statements) {
    ...
    if (statement.importClause.name) {
      imports.add(`flight.${moduleName}.default as ${statement.importClause.name.text}`);
    }
    const bindings = statement.importClause.namedBindings;
    ...
    for (const element of bindings.elements) {
      ...
      imports.add(`flight.${moduleName}.${importedName}${alias}`);
    }
  }
}
