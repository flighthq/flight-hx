...
Reads, writes, and calls on an unbound checker-proven host receiver use direct Haxe member syntax. An explicitly mapped parameter can emit `image.width`, while a host value recovered through a structural `Dynamic` path emits `(cast value : flighthq._internal.dom.HTMLImageElement).width`. The cast is important: it makes Haxe resolve both the declaration and its member instead of silently accepting another dynamic lookup. Optional access evaluates the receiver once, preserves the existing nullish result, and then uses the same typed field or method. Spread calls use `Reflect.callMethod` only for JavaScript argument-spread semantics; the method value itself is still read as a direct typed field, not by a string-named `_Runtime` lookup.

Existing target-semantic host endpoints retain priority over this ordinary direct route. Canvas2D, canvas-element, DOM-root, WebGL, and selected WebGPU operations still cross their maintained typed backend because that seam owns native portability or observable host semantics. The new mapping replaces the former `_Runtime.field`, `_Runtime.setField`, and `_Runtime.callProperty` fallback for other mapped host members; it does not bypass an already required backend.
...
The generated Haxe tree is `eb97d380f3b8f0f2078c2a6550b2ab67022e6bfcd0f0c1e7ff5a444cc3d58399`. The combined-tree JavaScript bundle is `c0e7de26371748c6dda6ef91b2fd4de131ee510d46ef3c44c153bd735fcfc888`. The machine-readable host report is `1647eaa52e9cab123c9e13c752d5cb3eeac517719961f91ac90f69d569916fdd`.

`npm run check` and all 127 generator tests pass. With review's exact 194-file maintained declaration census temporarily combined, the full Haxe namespace passes on Eval and JavaScript, and `CoreSmoke` passes on Eval, JavaScript, Python, and C++/hxcpp. That gate also made declaration arity and real-JavaScript extern member gaps fail visibly; their exact corrections remain owned by review and are not part of this generator commit.
