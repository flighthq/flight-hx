...
- The separate typed-struct class-emission audit now has Gates 1–3 implemented for the cpp-only `Camera2D` pilot. One exact canonical ID emits as a deterministic `@:structInit` class under `#if cpp` and retains the existing typedef elsewhere; no other schema is enabled. The focused nominal-construction smoke and byte-identical JavaScript fixture pass, and the full generated tree moved from SHA-256 `7fb39b6fb779838926fded9d2aa44ff31f2ca54f7778801660fa046c7ea9914f` to `fd76c64a2957d9d52a31c1f5b6d8a900271bb56bb8e92dc41c18998195aaecb4`. Native cpp compilation, the full generated-JavaScript-tree SHA gate, and review's five-run A/B benchmark remain pending on the integrated branch because this isolated base lacks a C++ compiler and predates the DOM backend fix. Particle classes remain a separate contingent Gate 4 change.
