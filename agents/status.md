...
- `npm run check`, 77 focused generator tests, full Eval namespace compilation, all 278 upstream math tests, all 996 upstream geometry tests, and the indexed endpoint smoke on Eval/JavaScript/Python pass. C++ portability remains unavailable because this host has no C++ compiler.
...
- Focused generator tests: 77 passing across eight files when the stale module-identity fixture is excluded, including deterministic output, patch drift, executable Haxe, typed receivers, and indexed-lowering coverage.
