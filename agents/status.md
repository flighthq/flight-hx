...
- Checker-proven indexed access now uses typed inline endpoints for `Array` and all nine supported typed-array families. Exact final-output accounting records 4,521 direct reads and 2,915 direct writes; 783 `_Runtime.getIndex` and 141 `_Runtime.setIndex` calls remain for mixed, optional, synthetic, and otherwise dynamic access. JavaScript keeps raw fractional keys, portable targets retain `Std.int` keys, setters return the original right-hand value, and compound updates evaluate receiver/key once before the right-hand side.
- Review's Step 2 `camera2d` Neko script benchmark improved from about 15.2 ms to 8.8 ms per frame (42% lower script cost, roughly 75% higher throughput). Step 3 must be measured against that new baseline by the review harness; the isolated builder clone does not contain its script-only camera runner.
...
- `npm run check`, 74 focused generator tests, full Eval namespace compilation, all 278 upstream math tests, all 996 upstream geometry tests, and the indexed endpoint smoke on Eval/JavaScript/Python pass. C++ portability remains unavailable because this host has no C++ compiler.
...
- Focused generator tests: 74 passing across eight files when the stale module-identity fixture is excluded, including deterministic output, patch drift, executable Haxe, typed receivers, and indexed-lowering coverage.
