...
- Native typed arrays: maintained Float32/Int16/UInt16 wrappers use `#if js` browser storage, `#elseif lime` Lime-native storage, and the existing generic array fallback. Generated Float32 constructors now reach that wrapper, runtime helpers preserve its semantics, and native GL dispatch unwraps uploads to the Lime view. The focused fake-Lime Eval smoke, full Eval namespace compile, JS/Python portability smokes, generator drift check, and upstream math parity pass; real Lime desktop compilation remains pending because the pinned Lime library is not installed locally.
