...
Each package's `./contract` export lane is Flight's protected inter-package channel. The generator still inventories every contract export and emits its JavaScript source bridge, but it does not emit a Haxe module for a contract source barrel that has no declarations of its own; `reports/core.json` records each omission and its manifest-derived reason. Across the complete export graph, a canonical declaration receives `@:noCompletion` only when it is reachable through a translated contract lane and through no non-contract lane in any package. Shared declarations such as `Vector2` remain visible, while contract-exclusive types and static members stay explicitly accessible without appearing in normal editor completion. Both decisions use manifest lane and canonical source identity, never a `Contract` filename or export-name allowlist.
