...
This declaration is also the portable ABI description for later Rust generation. Concrete factory-created `Entity` declarations use behavior-free nominal classes by default on every Haxe target, while `-D flight_struct_typedef` restores the structural typedef representation as a global compatibility and debug oracle. Generated constructors are sealed to their factory modules, `create<Type>` remains the public allocator, and fields remain public and mutable. Structural `Like`, output, and configuration seams remain typedefs or interfaces where callers genuinely construct or supply them. The generated provenance audit must prove exact factory identity and enumerate cross-schema transfer, dynamic ingress, containment, and observable aliasing before each concrete entity is activated on the nominal path.
